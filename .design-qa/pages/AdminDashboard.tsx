...
export default function AdminDashboard() {
    ...
    return (
        <div className="dashboard-page max-w-7xl mx-auto space-y-8 font-sans transition-all animate-in fade-in duration-500">

            {/* Page Header */}
            <PageHeader title="لوحة المتابعة اليومية" description={school?.name || "حضور الطلاب والغياب وحالة الشعب في مكان واحد"} icon={BarChart3} actions={<label className="school-date-field"><span><Calendar size={16} /> تاريخ المتابعة</span><input type="date" aria-label="تاريخ المتابعة" value={activeDate} onChange={e => { if(e.target.value) setDate(e.target.value); }} /></label>} />
