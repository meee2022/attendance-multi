...
export default function MessagesPage() {
  ...
  return (
    <div className="messages-page max-w-7xl mx-auto space-y-6 pb-20 font-sans animate-in fade-in duration-500">

      {/* Header */}
      <PageHeader title="رسائل أولياء الأمور" description="حدّد نوع الرسالة والصف والتاريخ، ثم راجع الرسائل وصدّرها للإرسال." icon={MessageSquare} actions={generated && <button onClick={() => exportMessagesExcel(generated.map(r => ({ phone: r.phone, message: r.message })), date, msgType)} className="school-button"><Download size={17} />تصدير الرسائل</button>} />
      ...
      <div className="bg-white rounded-2xl border border-qatar-gray-border qatar-card-shadow p-4 sm:p-5 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-qatar-gray-bg px-3 py-1.5 rounded-xl border border-qatar-gray-border text-sm mr-auto">
            ...
            <input
              type="date" aria-label="تاريخ الرسائل"
              value={date}
              onChange={e => {
