  ...
  return (
      <div className="app-shell min-h-screen bg-qatar-gray-bg text-qatar-ink font-sans" dir="rtl">
        <SchoolSetupGuard>
          ...
          <Navbar />
          <main id="main-content" tabIndex={-1} className="workspace max-w-7xl mx-auto py-4 xl:py-6 px-3 sm:px-6 xl:px-8 pb-28 xl:pb-10">
            <Routes>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4" dir="rtl">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-7 space-y-5 animate-in fade-in zoom-in-95 duration-200">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm font-bold text-qatar-gray-text leading-relaxed">
          ...
          <p className="text-[11px] font-bold text-qatar-gray-text">
...
function Navbar() {
  ...
  return (
    <nav aria-label="التنقل الرئيسي" className="bg-white shadow-sm border-b border-slate-200 sticky top-0 z-20" dir="rtl">
      {/* Top accent bar */}
      <div className="h-1 w-full" style={{ background: "var(--gradient-primary)" }} />

      <div className="max-w-7xl mx-auto px-3 sm:px-6 xl:px-8">
...
function BottomNav() {
  ...
  return (
    <>
      {showAdminDrawer && (
        <div className="xl:hidden fixed inset-0 z-40" dir="rtl">
          ...
          <div className="absolute bottom-16 right-0 left-0 bg-white rounded-t-3xl shadow-2xl border-t border-slate-100 animate-in slide-in-from-bottom-4 duration-300">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <button aria-label="إغلاق قائمة الإدارة" onClick={() => setShowAdminDrawer(false)} className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center">
                <X className="w-4 h-4 text-qatar-gray-text" />
              </button>
            </div>
      ...
      <nav aria-label="التنقل الرئيسي" className="school-bottom-nav xl:hidden fixed bottom-0 right-0 left-0 z-30 bg-white border-t-2 border-qatar-maroon shadow-[0_-4px_20px_rgba(0,0,0,0.08)]" dir="rtl">
        <div className="flex items-stretch h-16">
          {PUBLIC_NAV.map(({ to, icon, label }) => {
            ...
            return (
              <Link key={to} to={to} aria-current={isActive(to) ? "page" : undefined}
                className={`flex-1 flex flex-col items-center justify-center gap-0.5 text-[10px] font-bold transition-colors ${active ? "text-qatar-maroon" : "text-qatar-gray-text"
                  }`}
              >
          ...
          <button onClick={() => setShowAdminDrawer(v => !v)}
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 text-[10px] font-bold transition-colors relative ${isAdminRouteActive || showAdminDrawer ? "text-qatar-maroon" : "text-qatar-gray-text"
              }`}
          >
